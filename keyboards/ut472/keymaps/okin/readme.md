...
This is a German Keymap for the UT47.2.
It makes use of double tap for accessing German Umlauts and special characters usually not reachable on the small keyboard.

The RGB underglow feature is used to highlight what layer is currently active.
This will even work when RGB effects are disabled.
...
* TAB (green): General navigation, volume and media control
* Left (purple): F-keys
* Right (cyan): Numbers on the first row, controlling RGB light
* Numpad (white right side): Numbers on the first row aswell as a numpad block to the right.
